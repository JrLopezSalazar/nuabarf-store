...
const ShoppingCart = () => {
  ...
  // Efecto para obtener el carrito cuando el componente se monta
  useEffect(() => {
    // No es necesario establecer el estado local, ya que estamos usando el estado global del carrito directamente
  }, []);
      ...
      <section className="h-screen bg-gray-100 pt-20">
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
              ...
              <div key={item.id} className="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
                ...
                <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                  ...
                  <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                    ...
                    <p className="text-sm">{(item.quantity * parseFloat(item.product.price.replace('s/.', ''))).toFixed(2)}</p>
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
            ...
            <button className="mt-6 w-full rounded-md bg-orange-500 py-1.5 font-medium text-blue-50 hover:bg-blue-600">Check out</button>
          </div>
        </div>
      </section>
    </>
